...
async def upload_data(
    ...
    if not validation_result.get("success"):
        return JSONResponse(
            ...
            content={"success": False, "error": validation_result.get("error")},
        )

    project_dir_path = ProjectController().get_project_path(project_id=project_id)

    file_path, file_id = DataController().generate_unique_file_path(
        origin_file_name=file.filename, project_id=project_id
    )
...
async def possess_endpoint(project_id: str, process_request: ProcessRequest):
    ...
    if file_chunks is None or len(file_chunks) == 0:
        return JSONResponse(
            ...
            content={
                "success": False,
                "error": response_message_enums.PROCESS_FILE_CONTENT_FAILED.value,
            },
        )
